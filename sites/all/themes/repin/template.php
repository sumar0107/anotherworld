 ...
 */
  ...
  function repin_preprocess_html(&$variables, $hook) {
//drupal_add_js("
//    var _gaq = _gaq || [];
 ...
 */
function repin_preprocess_page(&$variables, $hook) {
  global $theme_path;
  ...
  if (isset($variables['node'])) {
    $variables['title'] = '';
    if (isset($variables['node']->field_category) && !empty($variables['node']->field_category)) {
      $node_wrapper = entity_metadata_wrapper('node', $variables['node']);
      $class = '';
      if (function_exists('repin_css_class')) {
        $class = repin_css_class($node_wrapper->field_category->name->value());
      }
  ...
  $variables['editor_admin_menu'] = str_replace('class="menu"', 'class="menu dropdown"', _repin_block('navigation', 'system'));
}
 ...
 */
function repin_preprocess_node(&$variables, $hook) {
 ...
 */
function repin_preprocess_comment(&$variables, $hook) {
 ...
 * 
 */
function _repin_block($bid, $module = 'block') {
...
function repin_links__system_secondary_menu(&$variables) {
  global $user;
  $html = "<div>\n";
  $html .= "  <ul>\n";
  $k = 0;
...
function repin_form_alter(&$form, $form_state, $form_id) {
...
function repin_preprocess_taxonomy_term(&$variables, $hook) {
  $variables['class_h1'] = '';
...
function repin_preprocess_breadcrumb(&$variables, $hook) {
...
function repin_preprocess_field(&$variables, $hook) {
